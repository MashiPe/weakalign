...
inliersHom = WeakInlierCount(geometric_model='hom',**arg_groups['weak_loss'])
...
def inlier_score_function(theta_hom,corr_hom,minimize_outliers=False):
    # inliers_comp = inliersComposed(matches=corr_aff,
    #                                              theta_aff=theta_aff,
    #                                              theta_aff_tps=theta_aff_tps)
    inliers_hom = inliersHom(matches = corr_hom,
                              theta = theta_hom)
...
def loss_fun(batch):
    ...
    theta_hom,corr_hom=model(batch)
    ...
    inlier_score_pos = inlier_score_function(theta_hom,corr_hom)
...
# TODO: prepare train-pair streetview
...
stats=compute_metric(metric,model,dataset_eval,dataloader_eval,batch_tnf,4,two_stage,do_aff,do_tps,do_hom,args)
eval_value=np.mean(stats['hom'][metric][eval_idx])
